...
Edim 의 모든 기능은 각각의 패키지로 모듈화되어 있으며, 원하는 기능만을 사용하거나 직접 서식을 추가할 수 있습니다. 각 패키지의 사용법은 [패키지 목록](/docs/packages)를 참고하세요. 필요한 경우, ProseMirror 생태계의 다른 라이브러리와 함께 사용할 수 있습니다.
