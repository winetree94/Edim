...
`@edim-editor/preset` 은 에디터에서 사용할 서식인 `EDIM_PRESET_SCHEMA` 와 에디터의 UI/UX 를 담당하는 플러그인의 집합인 `edimPresetPlugins`를 제공합니다. 아래는 ProseMirror 의 API 를 통해 에디터 인스턴스를 생성하는 방법의 예시입니다.
...
**Edim**은 현재 프레임워크를 위한 별도의 추상화 계층을 제공하지 않습니다. 따라서 각 프레임워크에 맞는 ProseMirror 추상화 라이브러리를 함께 사용하는 것이 도움이 될 수 있습니다. 관련 프로젝트는 아래의 링크를 참조하세요. 
